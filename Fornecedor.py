...
# Configuração das tabelas e colunas esperadas (atualizado conforme as imagens)
SUPABASE_CONFIG = {
    "vendas": {
        "table": "VWSOMELIER",
...
def fetch_supabase_data(cache, table, columns_expected):
    key = f"{table}"
    if key in cache:
        return cache[key]
...
def main():
    ...
    if vendas_df.empty:
        st.warning("Não há vendas disponíveis.")
    else:
